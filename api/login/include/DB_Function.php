...
class DB_Functions {
    ...
    public function simpanUser($nama, $email, $password) {
        ...
        $stmt = $this->conn->prepare("INSERT INTO tbl_user(unique_id, nama, email, pass_asli, encrypted_password, salt) VALUES(?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $uuid, $nama, $email, $password, $encrypted_password, $salt);
        $result = $stmt->execute();
        $stmt->close();
